...
# --- 도구 처리 노드 함수
def tools_node(state: ResearcherState, config: RunnableConfig | None = None) ->  ResearcherState:
    """
    연구 조사 워크플로우에서 도구 실행을 담당하는 노드 함수  

    이 노드는 LLM이 이전 단계에서 요청한 도구 호출을 실제로 수행하고,  
    각 도구의 실행 결과를 `ToolMessage` 형태로 상태에 추가한다.  
    즉, 연구 조사 에이전트의 '도구 실행 단계(tool execution step)'를 담당한다.
